...
function fileKind(name = '') {
  return name.toLowerCase().endsWith('.pdf') ? 'PDF' : 'Word';
}
...
function createDownloadMarkup(url, filename, label) {
  if (!url || url === '#') {
    return `<span class="muted">${label}: ${filename}</span>`;
  }
  return `<a class="download-link" href="${url}" download="${filename}">${label}: ${filename}</a>`;
}
...
function baseTaskCard(task, metaLabel) {
  ...
  card.querySelector('.task-file').innerHTML = createDownloadMarkup(task.fileUrl, task.fileName, 'Download task file');
    ...
    if (!task.submissions.length) {
      ...
      task.submissions.forEach((submission, index) => {
        ...
        block.innerHTML = `
          ...
          <p>${createDownloadMarkup(submission.fileUrl, submission.fileName, 'Download learner file')}</p>
  ...
  tasks.forEach((task) => {
    ...
    submitForm.addEventListener('submit', (event) => {
      ...
      if (submission) {
        ...
        if (hasUpload) {
          submission.fileName = upload.name;
          submission.fileUrl = URL.createObjectURL(upload);
        }
      } else {
        task.submissions.push({
          ...
          fileUrl: URL.createObjectURL(upload),
          feedback: ''
        });
      }
    ...
    feedbackList.innerHTML = `
      <p>${submission ? createDownloadMarkup(submission.fileUrl, submission.fileName, 'Download your submission') : '<span class="muted">No submission yet.</span>'}</p>
...
taskForm.addEventListener('submit', (event) => {
  ...
  tasks.unshift({
    ...
    fileUrl: URL.createObjectURL(taskUpload),
